...
export function useNodeConnections(nodes:BoardNode[],view:Viewport,viewport:RefObject<HTMLDivElement|null>){
 ...
 function ports(node:BoardNode){return <>
  <button type="button" className={`node-port node-port-input ${draft&&draft.source!==node.id?'can-connect':''} ${draft?.target===node.id?'is-target':''} ${edges.some(edge=>edge.target===node.id)?'is-connected':''}`} data-node-input={node.id} aria-label={`Input of ${node.name}`} title="Input — drop a connection here" onPointerDown={event=>event.stopPropagation()} onClick={()=>{if(draft)connect(draft.source,node.id);}}/>
  <button type="button" className={`node-port node-port-output ${draft?.source===node.id?'is-target':''} ${edges.some(edge=>edge.source===node.id)?'is-connected':''}`} aria-label={`Connect from ${node.name}`} title="Output — drag to another node's input"
